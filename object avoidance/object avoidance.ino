...
void loop() {
  currentTimer = millis();
  distanceFL = (float)sonar[2].ping_median(5) * factor;
  distanceFR = (float)sonar[3].ping_median(5) * factor;
  
  if (distanceFR <= 40 || distanceFL <= 40) {
    moveBackward();
    delay(100);
    stopMoving();
    delay(500);
    ...
    stopMoving();
    delay(300);
    if (currentTimer - previousTimer >= 1UL) {
      distanceFL = (float)sonar[2].ping_median(5) * factor;
      distanceFR = (float)sonar[3].ping_median(5) * factor;
    ...
    if (value == 0) {
      distanceR = (float)sonar[0].ping_median(5) * factor;
      distanceL = (float)sonar[1].ping_median(5) * factor;
    ...
    if (distanceR >= distanceL) {
      if (distanceR < 20) {
        ...
        Serial.println("Now rotate right ");
        rotateRight();
        delay(n1);
      }
      value = 0;
    ...
    if (distanceL > distanceR) {
      if (distanceL < 20) {
        ...
        Serial.println("Now rotate left");
        rotateLeft();
        delay(n1);
      }
      value = 0;
...
void moveForward() {
  Fmotors.setSpeedA(165);
  Fmotors.setSpeedB(165);
  Rmotors.setSpeedA(165);
  Rmotors.setSpeedB(165);
  Fmotors.backwardA();
  Fmotors.backwardB();
  Rmotors.forwardA();
  Rmotors.backwardB();
